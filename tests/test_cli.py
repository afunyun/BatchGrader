...
from batchgrader import cli  # Removed src. prefix
...
def mock_batch_runner_functions(mocker):
    mock_run_batch = mocker.patch(
        "batchgrader.cli.run_batch_processing"
    )  # Removed src. prefix
    mock_run_count = mocker.patch(
        "batchgrader.cli.run_count_mode"
    )  # Removed src. prefix
    mock_run_split = mocker.patch(
        "batchgrader.cli.run_split_mode"
    )  # Removed src. prefix
    return {"batch": mock_run_batch, "count": mock_run_count, "split": mock_run_split}


@pytest.fixture
def mock_load_config(mocker):
    return mocker.patch(
        "batchgrader.cli.load_config",  # Corrected src. prefix
        return_value={"test_key": "test_value"},
    )


def test_cli_batch_mode_input_file(
    mock_sys_argv, mock_batch_runner_functions, mock_load_config, tmp_path
):
    ...
    with pytest.raises(SystemExit) as e:  # cli.main calls sys.exit(0) on success
        cli.main()
...
def test_cli_count_mode_input_dir(
    mock_sys_argv, mock_batch_runner_functions, mock_load_config, tmp_path
):
    ...
    assert config == {"test_key": "test_value"}  # mock_load_config returns this
    mock_batch_runner_functions["batch"].assert_not_called()
    mock_batch_runner_functions["split"].assert_not_called()


def test_cli_split_mode_log_dir(
    mock_sys_argv, mock_batch_runner_functions, mock_load_config, tmp_path
):
    input_file = tmp_path / "split_me.xlsx"
    input_file.write_text(
        "dummy excel content"
    )  # Actual content not vital for CLI test
...
def test_cli_missing_input_defaults_to_input_dir(
    monkeypatch, mock_sys_argv, mock_batch_runner_functions, mock_load_config, tmp_path
):
    # Simulate input/ directory with a file
    input_dir = tmp_path / "input"
    input_dir.mkdir()
    (input_dir / "data.csv").write_text("id,text\n1,data")
    monkeypatch.setattr("batchgrader.constants.PROJECT_ROOT", tmp_path)
...
def test_cli_missing_input_and_empty_input_dir(
    monkeypatch,
    mock_sys_argv,
    mock_batch_runner_functions,
    mock_load_config,
    tmp_path,
    capfd,
):
    ...
    assert (
        "missing or empty" in out or "missing or empty" in err
    ), "Did not log error message for missing/empty input dir."
...
def test_cli_default_output_dir(
    ...
    )  # cli.py passes None if not specified, batch_runner handles default


def test_cli_load_config_file_not_found(
    mock_sys_argv, mock_batch_runner_functions, mocker, tmp_path
):
    input_file = tmp_path / "input.csv"
    input_file.write_text("id,text\n1,test")
    ...
    mock_logger = mocker.patch("batchgrader.cli.logger")  # Corrected src. prefix
    ...
    with pytest.raises(SystemExit) as e:
        cli.main()
    assert e.value.code == 0  # Should exit 0 as it proceeds with empty config
    ...
    assert any(
        "Configuration file not found" in call_args[0][0]
