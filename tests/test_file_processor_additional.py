...
def test_calculate_and_log_token_usage_minimal(monkeypatch, caplog):
    ...
    caplog.set_level('INFO', logger=fp.logger.name)
    fp.calculate_and_log_token_usage(df, "prompt", "resp", None, "my-model", "my-key")
    assert len(calls) == 1
...
def test_calculate_and_log_token_usage_logging_error(monkeypatch, caplog):
    ...
    df = pd.DataFrame({'a': [1]})
    fp.calculate_and_log_token_usage(df, "prompt", "resp", None, "model", "key")
    assert any("[Token Logging Error]" in rec.message for rec in caplog.records)
