...
from batchgrader.evaluator import load_prompt_template
...
def test_load_prompt_template_missing_from_yaml_fallback_to_default():
    ...
    with patch("builtins.open", mock_open(read_data="{}")):
        with patch("yaml.safe_load", return_value={}):
            with patch(
                    "batchgrader.config_loader.DEFAULT_PROMPTS",
...
def test_load_prompt_template_fail_complete():
    ...
    with patch("builtins.open", mock_open(read_data="{}")):
        with patch("yaml.safe_load", return_value={}):
            with patch("batchgrader.config_loader.DEFAULT_PROMPTS", {}):
...
def test_load_prompt_template_yaml_error():
    ...
    with patch("pathlib.Path.exists",
               return_value=True), patch(
                   "builtins.open",
                   mock_open(read_data="invalid: yaml: content")), patch(
                       "yaml.safe_load",
                       side_effect=yaml.YAMLError("Invalid YAML")), patch(
                           "batchgrader.config_loader.DEFAULT_PROMPTS",
                           {"batch_evaluation_prompt": DEFAULT_PROMPT_TEXT},
                       ), patch("sys.stderr"):
        prompt = load_prompt_template("batch_evaluation_prompt")
...
def test_load_prompt_template_general_error():
    ...
    with patch("pathlib.Path.exists", return_value=True), patch(
            "builtins.open", mock_open(read_data="valid: yaml")), patch(
                "yaml.safe_load",
                side_effect=Exception("Unexpected error")), patch(
                    "batchgrader.config_loader.DEFAULT_PROMPTS",
                    {"batch_evaluation_prompt": DEFAULT_PROMPT_TEXT},
                ), patch("sys.stderr"):
        prompt = load_prompt_template("batch_evaluation_prompt")
