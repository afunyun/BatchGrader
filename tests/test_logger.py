...
def test_logger_init_creates_directory():
    ...
    mock_path_instance, mock_path_constructor, mock_file_handler, mock_rich_handler = (
        _mock_logger_setup())
    with (
            patch("batchgrader.logger.Path", mock_path_constructor),
            patch("batchgrader.logger.logging.getLogger") as mock_get_logger,
            patch("batchgrader.logger.logging.FileHandler",
                  return_value=mock_file_handler),
            patch("batchgrader.logger.RichHandler",
                  return_value=mock_rich_handler),
    ):
...
def test_logger_init_adds_handlers():
    ...
    with (
            ...
            patch("batchgrader.logger.logging.getLogger",
                  mock_get_logger_function),
            patch("batchgrader.logger.logging.FileHandler",
                  mock_file_handler_class),
            patch("batchgrader.logger.RichHandler", mock_rich_handler_class),
            patch("batchgrader.logger.datetime", mock_datetime),
    ):

        # Call the function under test
        setup_logging(log_dir=Path("/test/logs"))  # Pass Path object
        ...
        (mock_path_instance /
         f"batchgrader_run_{mock_dt_instance.strftime.return_value}.log")
...
def test_logger_skips_adding_handlers_if_already_exists():
    ...
    with (
            ...
            patch("batchgrader.logger.logging.getLogger") as mock_get_logger,
            patch("batchgrader.logger.logging.FileHandler", MagicMock()),
            patch("batchgrader.logger.RichHandler", MagicMock()),
            patch("batchgrader.logger.Path"),
    ):  # Mock Path constructor

        mock_logger_instance = MagicMock()
        mock_logger_instance.hasHandlers.return_value = (
            True  # Simulate already has handlers
        )
        mock_get_logger.return_value = mock_logger_instance
...
def test_logger_methods_call_underlying_logger():
    ...
    with (
            ...
            patch("batchgrader.logger.logging.FileHandler"),
            patch("batchgrader.logger.RichHandler"),
            patch("batchgrader.logger.datetime") as mock_dt,
    ):
...
def test_logger_integration(temp_log_dir):
    ...
    # The patches should refer to 'batchgrader.logger.datetime' if logger.py is the module where datetime is used by setup_logging
    with (
            patch("batchgrader.logger.datetime", mock_dt),
            patch("batchgrader.logger.RichHandler", mock_rich_handler_class),
            patch("batchgrader.logger.logging.FileHandler",
                  mock_file_handler_class),
    ):
