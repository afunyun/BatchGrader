...
{"custom_id": "req2", "response": {"body": {"choices": [{"message": {"content": "Test Response 2"}}]}}}"""
    ...
    with patch.object(client_under_test.client.files, 'content', return_value=mock_content_response):
...
def test_llm_parse_batch_output_file_critical_failure(llm_client_instance):
    ...
    with patch.object(client_under_test.client.files, 'content', side_effect=Exception("API network error")):
