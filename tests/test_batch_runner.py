...
def test_run_batch_processing_single_file(mocker, mock_args_input_file, basic_config):
    mock_process_file = mocker.patch('batch_runner.process_file', return_value=True)
    mocker.patch('batch_runner.prune_logs_if_needed') # Mock to avoid side effects
    ...
    mock_process_file.assert_called_once_with(
        ...
        str(Path(mock_args_input_file.output_dir)),
        basic_config
    )

def test_run_batch_processing_input_dir(mocker, mock_args_input_dir, basic_config):
    mock_process_file = mocker.patch('batch_runner.process_file', return_value=True)
    ...
    expected_calls = [
        call(str(Path(mock_args_input_dir.input_dir) / "file1.csv"), str(Path(mock_args_input_dir.output_dir)), basic_config),
        call(str(Path(mock_args_input_dir.input_dir) / "file2.jsonl"), str(Path(mock_args_input_dir.output_dir)), basic_config)
    ]
...
def test_process_file(mocker, mock_args_input_file, basic_config):
    mock_file_wrapper = mocker.patch('batch_runner.process_file_wrapper', return_value=True)
    mocker.patch('batch_runner.load_system_prompt', return_value="Test System Prompt Content")
    ...
    mocker.patch('batch_runner.LLMClient', return_value=mock_llm_client_instance)

    success = process_file(mock_args_input_file.input_file, mock_args_input_file.output_dir, basic_config)
...
def test_run_count_mode_single_file(mocker, mock_args_input_file, basic_config):
    ...
    mock_check_limits = mocker.patch('batch_runner.check_token_limits', return_value=(True, {'total': 10, 'average': 10.0, 'max': 10}))
...
def test_run_split_mode_single_file(mocker, mock_args_input_file, basic_config):
    mock_args_input_file.mode = 'split'
    mock_split_file = mocker.patch('batch_runner.split_file_by_token_limit', return_value=(["chunk1.csv"], 100))
