...
def test_count_tokens_in_content(mock_encoder):
    ...
    assert count_tokens_in_content(None, mock_encoder) == 1  # "None" as a string
...
def test_count_input_tokens(mock_encoder):
    ...
    assert count_input_tokens(row, system_prompt, 'text', mock_encoder) == expected_tokens
...
def test_count_completion_tokens(mock_encoder):
    ...
    assert count_completion_tokens(row, mock_encoder, field='custom_field') == 3
...
def test_create_token_counter(mock_encoder):
    ...
    token_counter = create_token_counter(system_prompt, response_field, mock_encoder)
    ...
    custom_counter = create_token_counter(system_prompt, response_field, mock_encoder, prompt_template=template)
...
def test_count_tokens_in_df(sample_df, mock_encoder):
    ...
    # Second row: "System\nAnother test with more words" = 7 tokens
